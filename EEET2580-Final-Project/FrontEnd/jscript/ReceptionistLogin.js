async function loginForReceptionist(event) {
    ...
    try {
        ...
        const response = await fetch('http://localhost:8081/receptionist/login', {
            method: 'POST',
            headers: {
        ...
        if (data.auth) {
            // If authentication is successful, redirect to the receptionist Home Page
            window.location.href = '../html/ReceptionistHomePage.html'; // Redirect to the home page
        } else {
