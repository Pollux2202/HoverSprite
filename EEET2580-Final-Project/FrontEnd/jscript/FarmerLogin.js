async function login(event) {
    ...
    try {
        ...
        const response = await fetch('http://localhost:8081/farmer/login', {
            method: 'POST',
            headers: {
        ...
        if (data.auth) {
            // If authentication is successful, redirect to the Farmer Home Page
            window.location.href = '../html/FarmerHomePage.html'; // Redirect to the home page
        } else {
